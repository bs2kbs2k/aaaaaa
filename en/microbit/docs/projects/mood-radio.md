...
We add a `||radio:on received number||` block that will run code whenever a new "mood" message comes in. The `receivedNumber` variable contains the numeric value that was sent. Since we've decided that `0` is **smiley**, we add a conditional `||logic:if then||` statement to show this icon.
