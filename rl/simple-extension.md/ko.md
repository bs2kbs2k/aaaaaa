...
We need to make an extension description file next. 이것을 `pxt.json` 라고 합니다. The file defines all the components of the extension. Our `pxt.json` contains a minimum of what's needed to define our extension. Copy the JSON here and save it as `pxt.json` in the `/tropic` directory.
